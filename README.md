...
A: 至少需要以下权限：
- **组织管理**: 用户-只读、组织成员-只读
- **项目协作**：项目-只读、工作项-只读、工作项类型-只读、工作项评论-只读（如需查看工作项评论功能）
- **代码管理**：代码仓库-只读、代码组-只读、分支-只读、合并请求-读写
...
- 🎯 **工作项数量与云效网页「概览」完全对齐（issue #1）**
  - 所有工作项列表/搜索入口（粘贴模板选择、AI 发送选单、搜索视图、智能粘贴等）默认排除「已结束」状态工作项，不再把已关闭/已取消的项混入数量
  - 按编号精确查找、或用户在筛选器显式指定状态时，保留原样不受影响
  - 工作项树分组直接显示服务端过滤后的精确总数（如「需求 (532)」），不再显示「50+」近似值，「加载更多」同步展示已加载/总数进度
  - 底层 `statusStage NOT_CONTAINS` 条件经真实 API 验证：`statusStage` 为正确字段名（`statusStageId` 无效）、排除后分页总数精确、项目自定义进行中阶段自动保留
- 🐛 修复旧 `getWorkItems` 列表路径未应用状态过滤的问题，统一统计口径

### 1.6.1 (2026-07-09)

- 🔍 **工作项数量对齐云效网页端**
  - 在工作项树中默认排除「已完成/已取消」等结束状态的工作项，使各类型数量与云效网页端「概览」一致
  - 使用 `statusStage` 的 `NOT_CONTAINS` 条件排除结束阶段，自动包含项目自定义的进行中阶段，无需逐个枚举
  - 「加载更多」同步应用过滤，保证翻页后仍只显示进行中的工作项
  - 缓存键带上过滤模式，切换配置后不会命中旧缓存
- ⚙️ **新增配置项**
  - `yunxiao.workitemExcludeTerminal`：是否排除结束状态工作项，默认开启
  - `yunxiao.workitemTerminalStatusStages`：自定义被视为结束的状态阶段 ID（默认 `4,5`，即已完成、已取消）
